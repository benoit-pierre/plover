...
def normalize_steno(strokes_string):
    ...
    for stroke in strokes:
        if theory.STENO_NUMBER_KEY in stroke:
            stroke = stroke.replace(theory.STENO_NUMBER_KEY, '')
            if not re.search('[0-9]', stroke):
                stroke = theory.STENO_NUMBER_KEY + stroke
        ...
        has_implicit_dash = bool(set(stroke) & theory.IMPLICIT_HYPHENS)
...
class Stroke:
    ...
    def __init__(self, steno_keys) :
        ...
        steno_keys.sort(key=lambda x: theory.STENO_KEY_ORDER.get(x, -1))
         
        # Convert strokes involving the number bar to numbers.
        if theory.STENO_NUMBER_KEY in steno_keys:
            numeral = False
            for i, e in enumerate(steno_keys):
                if e in theory.STENO_KEY_NUMBERS:
                    steno_keys[i] = theory.STENO_KEY_NUMBERS[e]
                    numeral = True
            if numeral:
                steno_keys.remove(theory.STENO_NUMBER_KEY)
        
        if steno_keys_set & theory.IMPLICIT_HYPHEN_KEYS:
            self.rtfcre = ''.join(key.strip('-') for key in steno_keys)
        else:
            pre = ''.join(k.strip('-') for k in steno_keys if k[-1] == '-' or 
                          k == theory.STENO_NUMBER_KEY)
        ...
        self.is_correction = (self.rtfcre == theory.UNDO_STROKE_STENO)
