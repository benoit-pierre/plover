...
def make_candidates_from_rules(word, suffix, check=lambda x: True):
    candidates = []
    for r in theory.ORTHOGRAPHY_RULES:
...
def _add_suffix(word, suffix):
    in_dict_f = lambda x: x in theory.ORTHOGRAPHY_WORDS

    candidates = []
    
    alias = theory.ORTHOGRAPHY_RULES_ALIASES.get(suffix, None)
    ...
    if candidates:
        candidates.sort(key=lambda x: theory.ORTHOGRAPHY_WORDS[x])
        return candidates[0]
