...
class StrokeDisplayDialog(wx.Dialog):
    ...
    def __init__(self, parent, config):
        ...
        self.all_keys = ''.join(x[0].strip('-') for x in 
                                sorted(theory.STENO_KEY_ORDER.items(),
                                       key=lambda x: x[1]))
        self.reverse_numbers = {v: k for k, v in theory.STENO_KEY_NUMBERS.items()}
    ...
    def paper_format(self, stroke):
        ...
        for key in keys:
            ...
            index = theory.STENO_KEY_ORDER[key]
...
class TestApp(wx.App):
    def OnInit(self):
        ...
        keys = theory.STENO_KEY_ORDER.keys()
        for i in range(100):
