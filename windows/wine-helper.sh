...
helper_dist()
{(
  manifest="$ploverdir/build/manifest"
  ...
  if [ -d "$ploverdir/.git" ]
  then
    git -C "$ploverdir" ls-files >"$manifest"
  elif [ -d "$ploverdir/.svn" ]
  then
    svn list -R "$ploverdir" >"$manifest"
  else
  ...
  # Build egg.
  info "building egg"
  python2 setup.py --quiet bdist_egg
  test -r "dist/plover-$version-py2.7.egg"

  # Create install data.
  info "running pyinstaller"
  # Don't use launch.py or Plover egg will not get picked up.
  cp -v launch.py build/
  (
